...
function SortableTask({ task, columnId }: { task: Task; columnId: string }) {
  ...
  const [curentTask, setCurrentTask] = useState<string | null>(null);
  ...
  async function handleUpdateTask(e: any) {
    ...
    if (taskData.title.trim() && curentTask === displayedTask.id) {
      ...
      //  const previous = { ...displayedTask };
      setDisplayedTask((prev) => ({
        ...prev,
        title: taskData.title,
        description: taskData.description ?? "",
        assignee: taskData.assignee ?? "",
        due_date: taskData.dueDate ?? prev.due_date,
        ...
        <CardContent className="p-3 sm:p-4">
          <div className="space-y-2 sm:space-y-3">
            ...
            <p className="text-xs text-gray-600 line-clamp-2">
              {displayedTask.description || "No description."}
            </p>

            {/* Task Meta */}
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1 sm:space-x-2 min-w-0">
                {displayedTask.assignee && (
                  <div className="flex items-center space-x-1 text-xs text-gray-500">
                    <User className="h-3 w-3" />
                    <span className="truncate">{displayedTask.assignee}</span>
                  </div>
                )}
                {displayedTask.due_date && (
                  <div className="flex items-center space-x-1 text-xs text-gray-500">
                    <Calendar className="h-3 w-3" />
                    <span className="truncate">{displayedTask.due_date}</span>
                  </div>
                )}
              </div>
              <div
      ...
      <Dialog open={isEditingTask} onOpenChange={setIsEditingTask}>
        <DialogContent className="w-[95vw] max-w-[425px] mx-auto">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-600">Edit task on the board</p>
          </DialogHeader>
          ...
          <form className="space-y-4" onSubmit={handleUpdateTask}>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Enter task description"
                rows={3}
                defaultValue={displayedTask?.description ?? ""}
              />
            </div>
            <div className="space-y-2">
              <Label>Assignee</Label>
              <Input
                ...
                name="assignee"
                placeholder="Who should do this?"
                defaultValue={displayedTask?.assignee ?? ""}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                defaultValue={
                  displayedTask?.due_date
                    ? displayedTask.due_date.split("T")[0]
                    : ""
                }
              />
            </div>
